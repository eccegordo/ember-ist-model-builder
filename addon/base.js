...
var IstModelBase = function(modelConfig) {
  ...
  for(var attrName in (modelConfig.attributes || {}) ){
    ...
    if (attrConfig.belongsTo !== undefined) {
      if (attrConfig.value) {
        // allow overriding the belongsTo
        newModel[attrName] = attrConfig.value;
      } else {
        newModel[attrName] = belongsTo(attrConfig.belongsTo, {async: async} );
      }
    ...
    if (attrConfig.hasOne !== undefined) {
      ...
      if (attrConfig.value) {
        ...
        newModel[attrName] = belongsTo(attrConfig.hasOne, {async: async} );
      }
      continue;// go to next attr
    }
    ...
    if (attrConfig.hasMany !== undefined) {
      ...
      if (attrConfig.value) {
        // allow overriding the hasMany
        newModel[attrName] = attrConfig.value;
      } else {
        newModel[attrName] = hasMany(attrConfig.hasMany, {async: async} );
      }
      continue;// go to next attr
    }
    ...
    if (attrConfig.value !== undefined){
      ...
      // Use standard ember attr
      newModel[attrName] = attr(attrConfig.type, {defaultValue: attrConfig.defaultValue});
    }
